...
from backend.models.mentors_db import Subject_m
from backend.api.student import mentor_api_blueprint
from backend import db

@mentor_api_blueprint.route('/add_subjects', methods=['POST'])
...
@mentor_api_blueprint.route('/add_subject', methods=['POST'])
def add_subject():
    # Extract subject data from the request
    data = request.json
    if not data:
...
@mentor_api_blueprint.route('/update_subject/<int:id>', methods=['PUT'])  
def update_subject(id):
    ...
    try:
        ...
        subject = Subject_m.query.get(id)
        if not subject:
...
@mentor_api_blueprint.route('/all_subjects', methods=['GET'])
def get_all_subjects():
    try:
        # Query all subjects from the database
        subjects = Subject_m.query.all()
